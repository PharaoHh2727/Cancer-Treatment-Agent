...
class CMSurvivalPredictor:
    ...
    def _load_heatmap_background(self, background_path: str | None) -> np.ndarray | None:
        ...
        if not path.exists():
            print(f"  [CMTA attention] Heatmap background not found: {path}")
            return None
        ...
        if path.suffix.lower() == ".png":
            try:
                ...
                print(f"  [CMTA attention] Failed to load PNG heatmap background {path}: {exc}")
                return None

        print(f"  [CMTA attention] Failed to load heatmap background: {path}")
        return None
    ...
    def _prepare_heatmap_canvas(
        ...
        if background is not None and level_dim is not None:
            ...
            metadata = {
                ...
                "level_dim": [int(level_dim[0]), int(level_dim[1])],
                "canvas_shape": [int(canvas_h), int(canvas_w)],
                "fallback_reason": None,
            }
            return canvas, draw_coords, patch_w, patch_h, metadata

        if background is not None:
            canvas = np.asarray(background, dtype=np.uint8).copy()
            canvas_h, canvas_w = canvas.shape[:2]
            extent = coords.max(axis=0) + patch_size
            scale_x = canvas_w / max(float(extent[0]), 1.0)
            scale_y = canvas_h / max(float(extent[1]), 1.0)
            draw_coords = coords.copy()
            draw_coords[:, 0] = draw_coords[:, 0] * scale_x
            draw_coords[:, 1] = draw_coords[:, 1] * scale_y
            patch_w = max(1, int(np.ceil(patch_size * scale_x)))
            ...
            metadata = {
                ...
                "level_dim": None,
                "canvas_shape": [int(canvas_h), int(canvas_w)],
                "fallback_reason": "missing_level_dim_used_coord_extent",
            }
            return canvas, draw_coords, patch_w, patch_h, metadata
        ...
        metadata = {
            ...
            "level_dim": [int(level_dim[0]), int(level_dim[1])] if level_dim is not None else None,
            "canvas_shape": [int(canvas_h), int(canvas_w)],
            "fallback_reason": "background_load_failed" if background_path else "missing_background_path",
        }
        return canvas, shifted_coords * scale, patch_px, patch_px, metadata
    ...
    def _render_omic_patch_heatmaps(
        ...
        base_canvas, draw_coords, patch_w, patch_h, canvas_metadata = self._prepare_heatmap_canvas(
            coords=coords,
            patch_size=patch_size,
            background_path=background_path,
            level_dim=level_dim,
        )
        self._last_heatmap_canvas_metadata = canvas_metadata
    ...
    def _build_attention_result(
        ...
        coords_array = None
        heatmap_paths: list[str] = []
        coerced_level_dim = self._coerce_level_dim(pathomics_level_dim)
        heatmap_canvas_metadata = {
            "background_loaded": False,
            "used_background": False,
            "level_dim": [int(coerced_level_dim[0]), int(coerced_level_dim[1])] if coerced_level_dim is not None else None,
            "canvas_shape": None,
            "fallback_reason": "heatmaps_not_rendered",
        }
        if pathomics_coords is not None:
            ...
            if coords_array.ndim == 2 and coords_array.shape[0] >= n_path_tokens:
                ...
                heatmap_canvas_metadata = getattr(self, "_last_heatmap_canvas_metadata", heatmap_canvas_metadata)
            else:
                print(
        ...
        summary_payload = {
            ...
            "heatmap_level_dim": heatmap_canvas_metadata.get("level_dim"),
            "heatmap_background_loaded": heatmap_canvas_metadata.get("background_loaded", False),
            "heatmap_used_background": heatmap_canvas_metadata.get("used_background", False),
            "heatmap_fallback_reason": heatmap_canvas_metadata.get("fallback_reason"),
            "heatmap_canvas_shape": heatmap_canvas_metadata.get("canvas_shape"),
